...
internal class DeclarationReferenceInfo(val scopeList: List<String>, val irDeclarationReference: IrDeclarationReference) {
    internal var calculatedListForImport = irDeclarationReference.obtainDefaultImportStatement()
    internal var calculatedName = irDeclarationReference.obtainDefaultName()
    ...
    companion object {
        private fun <T : IrDeclarationReference> T.obtainDefaultImportStatement(): List<String> {
            ...
            val ownersPackageStr = (owner as? IrDeclarationWithName)?.file?.fqName?.asString() ?: EMPTY_TOKEN
            val ownersFqNameStr = (owner as? IrDeclarationWithName)?.fqNameWhenAvailable?.asString() ?: EMPTY_TOKEN
