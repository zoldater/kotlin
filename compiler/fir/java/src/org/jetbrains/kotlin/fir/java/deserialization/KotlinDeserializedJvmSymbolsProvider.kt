...
class KotlinDeserializedJvmSymbolsProvider(
    ...
    private val classesCache = mutableMapOf<FirClassId, FirClassSymbol>()
    private val typeAliasCache = mutableMapOf<FirClassId, FirTypeAliasSymbol?>()
    private val packagePartsCache = mutableMapOf<FirFqName, Collection<PackagePartsCacheData>>()

    private inner class PackagePartsCacheData(
        ...
        val topLevelFunctionNameIndex by lazy {
            proto.functionList.withIndex()
                .groupBy({ context.nameResolver.getName(it.value.name).intern(session) }) { (index) -> index }
        }
    ...
    private fun hasTopLevelClassOf(classId: ClassId, firClassId: FirClassId): Boolean {
        val knownNames = knownClassNamesInPackage.getOrPut(firClassId.packageFqName) {
            javaClassFinder.knownClassNamesInPackage(classId.packageFqName)
        } ?: return false
        return classId.relativeClassName.topLevelName() in knownNames
    }

    private fun computePackagePartsInfos(packageFqName: FirFqName): List<PackagePartsCacheData> {

        return packagePartProvider.findPackageParts(packageFqName.toString()).mapNotNull { partName ->
            val classId = ClassId.topLevel(JvmClassName.byInternalName(partName).fqNameForTopLevelClassMaybeWithDollars)
            val firClassId = classId.intern(session)
            if (!hasTopLevelClassOf(classId, firClassId)) return@mapNotNull null
    ...
    override fun getClassLikeSymbolByFqName(classId: FirClassId): ConeClassLikeSymbol? {
        return findAndDeserializeClass(classId.asClassId(), classId) ?: findAndDeserializeTypeAlias(classId)
    }
    ...
    private fun ConeClassifierSymbol?.toDefaultResolvedTypeRef(classId: FirClassId): FirResolvedTypeRef {
        return this?.let {
    ...
    private fun loadAnnotation(
        annotationClassId: FirClassId, result: MutableList<FirAnnotationCall>
        ...
        return object : KotlinJvmBinaryClass.AnnotationArgumentVisitor {
            ...
            private fun ClassLiteralValue.toFirClassReferenceExpression(): FirClassReferenceExpression {
                val firClassId = classId.intern(session)
                val literalLookupTag = ConeClassLikeLookupTagImpl(firClassId)
                ...
                return FirFunctionCallImpl(session, null).apply {
                    ...
                    val entryCallableSymbol =
                        this@KotlinDeserializedJvmSymbolsProvider.session.service<FirSymbolProvider>().getClassDeclaredCallableSymbols(
                            this@toEnumEntryReferenceExpression.intern(session), firName
                        ).firstOrNull()
                    ...
                    this.calleeReference = when {
                        entryClassSymbol != null && (entryClassSymbol as? FirClassSymbol)?.fir is FirEnumEntry -> {
                            FirResolvedCallableReferenceImpl(
                                this@KotlinDeserializedJvmSymbolsProvider.session, null, firName, entryClassSymbol
                            )
                        }
                        entryCallableSymbol != null -> {
                            FirResolvedCallableReferenceImpl(
                                this@KotlinDeserializedJvmSymbolsProvider.session, null, firName, entryCallableSymbol
                            )
                        }
                        else -> {
                            FirErrorNamedReference(
                                this@KotlinDeserializedJvmSymbolsProvider.session, null,
                                errorReason = "Strange deserialized enum value: ${this@toEnumEntryReferenceExpression}.$firName"
                            )
                        }
                    }
                }
            ...
            override fun visitAnnotation(name: Name, classId: ClassId): KotlinJvmBinaryClass.AnnotationArgumentVisitor? {
                ...
                val visitor = loadAnnotation(classId.intern(session), list)!!
                ...
                result += FirAnnotationCallImpl(session, null, null, symbol.toDefaultResolvedTypeRef(annotationClassId)).apply {
                    for ((name, expression) in argumentMap) {
                        arguments += FirNamedArgumentExpressionImpl(
                            this@KotlinDeserializedJvmSymbolsProvider.session, null, name.intern(session), false, expression
                        )
                    }
                }
            }
    ...
    private fun loadAnnotationIfNotSpecial(
        ...
        if (annotationClassId.asClassId() in AbstractBinaryClassAnnotationAndConstantLoader.SPECIAL_ANNOTATIONS) return null
    ...
    private fun findAndDeserializeClass(
        ...
        if (!hasTopLevelClassOf(classId, firClassId)) return null
        if (classesCache.containsKey(firClassId)) return classesCache[firClassId]


//        return classesCache.getOrPut(firClassId) {
        //return null
        ...
        if (kotlinJvmBinaryClass == null) {
            ...
            val symbol = FirClassSymbol(firClassId)
            FirEnumEntryImpl(session, null, symbol, firClassId.shortClassName).apply {
            ...
            classesCache[firClassId] = symbol
        } else {
            ...
            kotlinJvmBinaryClass.loadClassAnnotations(object : KotlinJvmBinaryClass.AnnotationVisitor {
                override fun visitAnnotation(classId: ClassId, source: SourceElement): KotlinJvmBinaryClass.AnnotationArgumentVisitor? {
                    return loadAnnotationIfNotSpecial(classId.intern(session), annotations)
                }
        ...
        return classesCache[firClassId]
//        }
    }

    private fun loadFunctionsByName(part: PackagePartsCacheData, name: FirName): List<FirCallableSymbol> {
    ...
    private fun loadPropertiesByName(part: PackagePartsCacheData, name: FirName): List<FirCallableSymbol> {
    ...
    private fun getPackageParts(packageFqName: FirFqName): Collection<PackagePartsCacheData> {
    ...
    override fun getAllCallableNamesInPackage(fqName: FirFqName): Set<FirName> {
        return getPackageParts(fqName).flatMapTo(mutableSetOf()) { packagePart ->
            packagePart.proto.functionList.map { packagePart.context.nameResolver.getName(it.name).intern(session) }
        }
    }

    override fun getClassNamesInPackage(fqName: FirFqName): Set<FirName> =
        javaClassFinder.findPackage(FqName.fromSegments(fqName.segments().map { it.asString() }))
            ?.getClasses { true }.orEmpty()
            .mapTo(sortedSetOf(), { it.name.intern(session) })
    ...
    override fun getPackage(fqName: FirFqName): FirFqName? = null
