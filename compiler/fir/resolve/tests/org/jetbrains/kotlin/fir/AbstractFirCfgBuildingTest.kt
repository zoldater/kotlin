...
abstract class AbstractFirCfgBuildingTest : AbstractFirResolveTestCase() {
    ...
    fun checkCfg(path: String, firFiles: List<FirFile>) {
        ...
        val simpleCfgDump = simpleBuilder.toString()
        val simpleExpectedPath = path.replace(".kt", ".cfg.txt")
        KotlinTestUtils.assertEqualsToFile(File(simpleExpectedPath), simpleCfgDump)
