...
class LockBasedLazyResolveStorageManager(private val storageManager: StorageManager) : StorageManager by storageManager,
    ...
    override fun <K, V : Any> createSoftlyRetainedMemoizedFunction(compute: Function1<K, V>): MemoizedFunctionToNotNull<K, V> =
        storageManager.createMemoizedFunction<K, V>(compute, ContainerUtil.createConcurrentSoftValueMap<K, Any>())

    override fun <K, V : Any> createSoftlyRetainedMemoizedFunctionWithNullableValues(compute: Function1<K, V>): MemoizedFunctionToNullable<K, V> =
