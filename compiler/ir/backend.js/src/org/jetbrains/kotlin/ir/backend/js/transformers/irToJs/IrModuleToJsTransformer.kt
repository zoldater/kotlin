...
class IrModuleToJsTransformer(
    ...
    private fun processClassModels(
        classModelMap: Map<IrClass, JsIrClassModel>,
        preDeclarationBlock: JsBlock,
        postDeclarationBlock: JsBlock
    ) {
        val declarationHandler = object : DFS.AbstractNodeHandler<IrClass, Unit>() {
            override fun result() {}
            override fun afterChildren(current: IrClass) {
    ...
    fun IrDeclarationWithName.isExported(): Boolean {
        if (fqNameWhenAvailable in backendContext.exportedDeclarations)
            return true
