...
/** Builds a [ProgressionHeaderInfo] for progressions built using the `step` extension function. */
        ...
        with(context.createIrBuilder(scopeOwner, expression.startOffset, expression.endOffset)) {
            ...
            val throwIllegalStepExceptionCall = {
                irCall(context.irBuiltIns.illegalArgumentExceptionSymbol, stepType).apply {
                    ...
                    exceptionMessage.addArgument(stepArgExpression)
            ...
            val checkedStepExpression = when {
                stepArgValueAsLong == null -> {
                    ...
                    val stepPositiveCheck = irCall(stepGreaterFun).apply {
                        putValueArgument(0, stepArgExpression)
                        putValueArgument(1, zeroStep)
                    }
                    irIfThenElse(
                        stepType,
                        stepPositiveCheck,
            ...
            val checkedAndMaybeNegatedStep = when (nestedInfo.direction) {
                ...
                ProgressionDirection.UNKNOWN -> {
                    ...
                    irIfThenElse(stepType, nestedStepPositiveCheck, checkedStepOrGet, checkedStepOrGet.negate())
                }
            }
